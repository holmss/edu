...
template <class T>
TStackItem<T>::TStackItem(const std::shared_ptr<T>& itm)
    : item(itm)
    , next(nullptr)
...
template <class T>
TAllocationBlock TStackItem<T>::stackitem_allocator(sizeof(TStackItem<T>), 100);
...
template <class T>
void* TStackItem<T>::operator new(size_t)
{
